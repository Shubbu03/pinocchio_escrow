...
use pinocchio_escrow::instructions::Initialize;
use pinocchio_escrow::states::{to_bytes, EscrowState};
use solana_sdk::rent::Rent;
use solana_sdk::sysvar::Sysvar;

pub const PROGRAM: Pubkey = pubkey!("GbqefpNQgSDkGj3Yv3zdtUiVD9qgZo6LGw3ZTeBJgbWP");
...
#[test]
fn test_initialize_EscrowState() {
    let mollusk = mollusk();
    ...
    // Create the PDA
    let (EscrowState_pda, bump) =
        Pubkey::find_program_address(&[EscrowState::SEED.as_bytes(), &PAYER.to_bytes()], &PROGRAM);

    //Initialize the accounts
    let payer_account = Account::new(1 * LAMPORTS_PER_SOL, 0, &system_program);
    let EscrowState_account = Account::new(0, 0, &system_program);
    ...
    let ix_accounts = vec![
        ...
        AccountMeta::new(EscrowState_pda, false),
    ...
    let tx_accounts = &vec![
        ...
        (EscrowState_pda, EscrowState_account.clone()),
